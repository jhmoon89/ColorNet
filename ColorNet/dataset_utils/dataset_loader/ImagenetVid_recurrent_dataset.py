...
class imagenetVidDataset(object):
    ...
    def _loadDataPointPath(self):
        ...
        for subtrainpath in subtrainPathList:
            ...
            for k in sub_sub_train_path:
                ...
                if len(imgPathList) < self._concurrentLength + 1:
                    continue
                for img in range(len(imgPathList) - self._concurrentLength):
    ...
    def getConcurrentImages(self, path, startind):
        ...
        output_box = np.zeros([self._cellNum[0], self._cellNum[1], 4 * self._B, self._concurrentLength], np.float32)  ### 13 by 13 by 4B by T
        output_vel = np.zeros([self._cellNum[0], self._cellNum[1], 2 * self._B, self._concurrentLength], np.float32)   ### 13 by 13 by 2B by T
        output_conf = np.zeros([self._cellNum[0], self._cellNum[1], self._B, self._concurrentLength], np.float32)  ### 13 by 13 by B by T
        output_cls = np.zeros([self._cellNum[0], self._cellNum[1], self._classNum, self._concurrentLength],
                              np.float32)  ### 13 by 13 by classNum
        final_output = np.zeros([self._cellNum[0], self._cellNum[1], self._outputDim, self._concurrentLength], np.float32)
        ...
        for i in range(self._concurrentLength):
            ...
            img_name = path + '/' + index + '.JPEG'
            # next frame
            next_index = str(startind + i + 1).zfill(6)
            next_img_name = path + '/' + next_index + '.JPEG'
            ...
            ### Read xml for current frame
            xml_path = str.replace(img_name, 'Data', 'Annotations')
            xml_path = str.replace(xml_path, 'JPEG', 'xml')

            ### xml for next frame
            next_xml_path = str.replace(next_img_name, 'Data', 'Annotations')
            next_xml_path = str.replace(next_xml_path, 'JPEG', 'xml')
            ...
            # xml info from current image frame
            ...
            # xml info from next frame
            next_xml_file = parse(next_xml_path)
            next_note = xml_file.getroot()
            next_obj_list_temp = next_note.findall('object')

            next_info = []
            for next_obj in next_obj_list_temp:
                xmax = int(next_obj.find('bndbox').findtext('xmax'))
                xmin = int(next_obj.find('bndbox').findtext('xmin'))
                ymax = int(next_obj.find('bndbox').findtext('ymax'))
                ymin = int(next_obj.find('bndbox').findtext('ymin'))
                trackid = int(next_obj.findtext('trackid'))
                next_info.append(np.array([trackid, xmax, xmin, ymax, ymin]))
            next_info = np.asarray(next_info)
            ...
            for j in range(obj_num):
                ...
                x_offset = (xc - nx * float(self._imageSize[0] / self._cellNum[0])) / float(
                    self._imageSize[0] / self._cellNum[0])
                y_offset = (yc - ny * float(self._imageSize[1] / self._cellNum[1])) / float(
                    self._imageSize[1] / self._cellNum[1])

                # print "before"
                # print (ny, nx)

                conf_nonzero_ind = (output_conf[ny, nx, :, i] == 0).argmax(axis=-1)
                if conf_nonzero_ind >= self._B:
                    continue

                output_conf[ny, nx, conf_nonzero_ind, i] = 1
                box = np.array([x_offset
                               ...
                               , np.float32)
                # print box
                output_box[ny, nx, conf_nonzero_ind * 4:(conf_nonzero_ind + 1) * 4, i] = box
                output_cls[ny, nx, obj_ind, i] = 1
                # print output_cls[ny, nx, :]

                ### info from next frame
                if np.isin(trackid, next_info[:, 0]):
                    next_ind, = np.where(trackid == next_info[:, 0]) # find the same trackid on the next frame
                    assert len(next_ind) == 1
                    next_box = next_info[next_ind[0], 1:]
                    next_xc = ((next_box[0] + next_box[1]) / 2.0) * img_resize_scale[0]
                    next_yc = ((next_box[2] + next_box[3]) / 2.0) * img_resize_scale[0]
                    xv = (next_xc - xc) / self._imageSize[0]
                    yv = (next_yc - yc) / self._imageSize[1]
                    output_vel[ny, nx, conf_nonzero_ind * 2:(conf_nonzero_ind + 1) * 2, i] = [xv, yv]

            final_output = np.concatenate([output_box, output_vel, output_conf, output_cls], -2)
...
# sample_img_path = '/ssdubuntu/data/ILSVRC/Data/VID/train/ILSVRC2015_VID_train_0000/ILSVRC2015_train_00010001'
# sample_xml_path = '/ssdubuntu/data/ILSVRC/Annotations/VID/train/ILSVRC2015_VID_train_0000/ILSVRC2015_train_00010001/000009.xml'
#
# xml_file = parse(sample_xml_path)
# note = xml_file.getroot()
# obj_list_temp = note.findall('object')
# id_list_temp = obj_list_temp[1].findtext('trackid')
#
# # print obj_list_temp
# # print id_list_temp
#
# next_info = []
# for next_obj in obj_list_temp:
#     xmax = int(next_obj.find('bndbox').findtext('xmax'))
#     xmin = int(next_obj.find('bndbox').findtext('xmin'))
#     ymax = int(next_obj.find('bndbox').findtext('ymax'))
#     ymin = int(next_obj.find('bndbox').findtext('ymin'))
...
# next_info = np.asarray(next_info)
#
# ind, = np.where(3 == next_info[:, 0])
# print ind

# print next_info
# ET.dump(obj_list_temp[0])
...
sample = vid_data.getNextBatch(3)
