...
gamut_range = np.tile(np.expand_dims(np.expand_dims(gamut_range, 0), 0), [img_size, img_size, 1, 1])
...
for subtrainpath in subtrainPathList:
    ...
    Bin_xxxx = xxxx.replace("/ssdubuntu/data/ILSVRC/Data/VID/train", "/ssdubuntu/Bin_Data")
    ...
    for k in sub_sub_train_path:
        ...
        Bin_semi = semi_finalPath.replace("/ssdubuntu/data/ILSVRC/Data/VID/train", "/ssdubuntu/Bin_Data")
        ...
        for img_ind in range(len(imgPathList)):
            ...
            img_tile = np.tile(np.expand_dims(img[..., 1:] / 16, -2), [1, 1, bin_Num, 1])
            mask = np.equal(img_tile, gamut_range)
            mask = np.all(mask, -1).astype('int')
            mask_ind = mask.argmax(-1)
            # print mask_ind
            np.save(os.path.join(Bin_semi, imgPathList[img_ind]).replace(".JPEG", "_small_cell16.npy"),
                    mask_ind.astype('int8'))
        print k
print 'load done!'
