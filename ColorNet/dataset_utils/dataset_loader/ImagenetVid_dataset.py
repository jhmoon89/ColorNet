...
#             print LastFrame_Output
    ...
    def get_video_data(self, frame_num=None):
        # subpath_rand = np.random.randint(0, len(self._subtrainPathList))
        # subsubpath_rand = np.random.randint(0, len(self._subsubtrainPathList[subpath_rand]))
        #
        # print subpath_rand, subsubpath_rand
        # print os.path.join(self._trainPath,
        #                    self._subtrainPathList[subpath_rand],
        #                    self._subsubtrainPathList[subpath_rand][subsubpath_rand])
        path = self._dataPointPathList[np.random.randint(0, len(self._dataPointPathList))]
        print path
        parent_path = os.path.abspath(os.path.join(path, '..'))  # get the parent path
        ...
        if frame_num == None:
            real_frame_num = len(os.listdir(parent_path))
        else:
            real_frame_num = frame_num
        for ind in range(real_frame_num - self._concurrentLength):
            frame_list, LastFrame_Output = self.getConcurrentImages(parent_path, ind)
            each_frame_data = {
                'Paths': [path],
                'Images': np.expand_dims(np.array(frame_list), 0),
                'Outputs': np.expand_dims(np.array(LastFrame_Output), 0)
            }
            # print np.shape(each_frame_data['Images'])
            # print np.shape(each_frame_data['Outputs'])
            Data_Seq.append(each_frame_data)
...
#     # print i
#     img = cv2.imread(vid_data._dataPointPathList[i], cv2.IMREAD_COLOR)  # .astype('float')
#     img = cv2.resize(img, imageSize)
#     img = cv2.cvtColor(img, cv2.COLOR_BGR2Lab)
#     # img = cv2.cvtColor(img, cv2.COLOR_Lab2BGR)
#     Data = pd.DataFrame(np.stack([np.reshape(img[:, :, 1], -1), np.reshape(img[:, :, 2], -1)], -1))
#     Data_List = Data_List.append(Data)
#
# print Data_List
# save_path = "/ssdubuntu/color/data.csv"
# Data_List.to_csv(save_path)



# vid_data.get_video_data(frame_num=50)
